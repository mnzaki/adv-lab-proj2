...
public class GUI extends JFrame {
	...
	public GUI() {
		...
		btnEnterSpeech.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String grammar_content = "#JSGF V1.0;\n grammar TicTacToe;\n <numbers> = (THREE | FOUR | FIVE | SIX | SEVEN | EIGHT | NINE);\n public <command> = <numbers>;";
				try {
					File myFoo = new File(
							"src/apps/edu/cmu/sphinx/demo/tictactoe/tictactoe.gram");
					FileOutputStream fooStream = new FileOutputStream(myFoo,
							false);
					byte[] myBytes = grammar_content.getBytes();
				...
				int size = Numbers.valueOf(resultText).getValue();
				showGame(size);
			}
		});
